...
- **`state-summary.md` drift advisory header.** When the resolved Claude model ID differs from the baseline `/audit` entry's model (baseline derived via reverse-chronological scan of `config-changelog.md`), the generated `state-summary.md` injects a one-line header `Model drift: <baseline_model_id> -> <current_model_id>` immediately after the `# Claude Code Configuration State` H1, before `## Project Profile`. Silent on the non-drift states (`match`, `missing_baseline`, `normalization_null`). All four skills (`/audit`, `/create`, `/secure`, `/optimize`) render the same header through the shared renderer path — no per-skill branching in the derivation.
- **`plugin/references/learning-system.md § Drift Advisory Derivation` (new subsection).** Specifies the single derivation consumed by both render sinks (state-summary header + `/audit` terminal drift block). Algorithm: normalize the current model ID → reverse-chronological scan for the baseline `/audit` anchor (Recent Activity entries first, then Compacted History buckets with first-anchor-wins) → normalize baseline → four-branch silence evaluation order (`normalization_null` / `missing_baseline` / `match` / `drift`) → return state + raw model-ID strings for render. Transience clause (advisory never persists to `recommendations.json`). Stateless clause (state-summary header not rendered for any skill when `local/` is unwritable; `/audit` terminal drift block renders from an in-memory changelog snapshot when available). Cross-skill invariance clause. `learning-system.md` version `2.3.1 → 2.3.2`.
- **`.github/scripts/check-smoke-fixtures.py` helpers and unit tests.** New module-level helpers: `_scan_baseline_anchor(changelog_text)` (reverse-chronological scan with first-anchor-wins), `_drift_state(current_fp, baseline_present, baseline_fp)` (silence evaluation order), `_normalize_model_id(model_id)` (canonical 4-axis fingerprint lookup), and drift-header injection in `render_state_summary()`. New internal unit tests `_test_scan_order()`, `_test_render_drift_header()`, `_test_t6_fixtures()` wired into the main `check()` runner.
- **5 new `ci/fixtures/t6-*` scenarios.** `t6-drift-recent-activity`, `t6-drift-compacted-history`, `t6-normalization-null-silence`, `t6-crossskill-create-drift` (4 rendering fixtures byte-matched against `golden/state-summary.md`). `t6-stateless-silence` (structural marker fixture asserting no `state-summary.md` is produced in stateless mode). Fixtures use canonical 4-axis enums `{opus,sonnet,haiku} × {200k,1M} × {none,extended_any} × {manual,compaction_capable}` throughout.

### Changed

- **`plugin/skills/audit/SKILL.md` drift advisory block (L167–174) trimmed to terminal-sink-specific.** Previously carried the full derivation inline. Derivation now lives in shared `learning-system.md § Drift Advisory Derivation`; the block here contains only the `/audit` terminal render trigger + pointer to the shared reader-side derivation + transience clause + stateless terminal-retain clause. All pre-existing drift-aware assertions continue to pass.

...
`/claude-code-template:setup`을 실행하면, Claude가 대화형 인터뷰를 통해
모든 설정 파일을 자동 생성합니다.
...
2. **프로젝트에서 설정 커맨드를 실행**하세요:
   ...
   **대체 방법** (플러그인 설치 없이):
   ...
   | 절대 경로 | `@~/Claude-Code-Template/commands/setup.md` |
   | 직접 붙여넣기 | `commands/setup.md`의 내용을 복사하여 대화에 직접 붙여넣기 |
...
> 자동 생성합니다. 이후 `/claude-code-template:setup`에서 "기존 프로젝트"를
...
├── .claude-plugin/        ← 플러그인 매니페스트 (플러그인으로 설치 가능)
├── commands/
│   └── setup.md           ← 설정 커맨드 (/claude-code-template:setup)

...
例: `guardians-of-the-claude` プラグインは、プロジェクトプロファイル、判断記録、最新のスキル実行結果を `local/` サブディレクトリに保存します（`project-profile.md`、`config-changelog.md`、`latest-{skill}.md`）。これらのファイルにより、スキルはセッションをまたいでプロジェクトのコンテキスト・ユーザーの好み・保留中の推奨事項を覚えていられます。

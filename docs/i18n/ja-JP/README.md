...
> **Windows** では SessionStart フックが bash エントリと PowerShell エントリの両方を同梱しているため、**PowerShell 5.1+（Windows 10+ に標準搭載）または Git Bash/WSL** のいずれかがあればフックが動作します。**高度なテンプレートのスクリプト（`templates/advanced/scripts/*.sh`）は引き続き bash 互換シェルが必要です。**

...
│   │   └── scaffold-feature/
│   │       └── SKILL.md
│   └── .plugin-cache/            # 플러그인 상태 파일 (자동 생성, gitignore 대상)
│       └── <plugin-name>/
└── src/
    └── CLAUDE.md                 # 폴더별 지침 (필요 시 로드)
```
...
| 파일 | 제한 | 유형 | 동작 |
| ------ | ------- | ------ | ------------- |
| MEMORY.md | 200줄 | **엄격한 로딩 제한** | 200줄을 초과하는 내용은 세션 시작 시 로드되지 않습니다. 잘려나갑니다. |
| CLAUDE.md | 200줄 | **권장 가이드라인** | 파일 길이에 관계없이 전체가 로드됩니다. 하지만 짧은 파일일수록 지침 준수율이 높아집니다. |
...
일부 플러그인은 `.claude/.plugin-cache/<plugin-name>/`에 프로젝트별 상태를 저장합니다. 이 디렉토리는 플러그인에 의해 **자동 생성**되며, 수동으로 편집하거나 버전 관리에 커밋해서는 안 됩니다. 플러그인은 `.plugin-cache/` 내부에 자체 `.gitignore`를 관리하여 캐시 파일이 git에서 제외되도록 합니다.

예시: `guardians-of-the-claude` 플러그인은 프로젝트 프로파일, 결정 changelog, 누적된 권장사항을 `local/` 하위 디렉토리에 저장합니다 (`profile.json`, `recommendations.json`, `config-changelog.md`, 여기에 파생된 사람이 읽을 수 있는 `state-summary.md`). 이 파일들을 통해 스킬들이 프로젝트 맥락, 사용자 선호도, 보류 중인 권장사항을 세션 간에 기억할 수 있습니다.
...
| `.claude/.plugin-cache/` | 아니오 | 플러그인 관리 상태 파일 -- 자동 생성 |
...
Claude Code에는 세션 시작 시 모두 로드되지만 서로 다른 목적을 가진 네 가지 시스템이 있습니다:
...
| **플러그인 캐시** | 플러그인 | 플러그인이 관리하는 프로젝트별 상태 | `.claude/.plugin-cache/<plugin-name>/` |

핵심 요점: **CLAUDE.md는 사용자가 Claude에게 전달하는 내용입니다. 자동 메모리는 Claude가 스스로에게 전달하는 내용입니다. 플러그인 캐시는 플러그인이 스스로에게 전달하는 내용입니다.** 각 시스템은 서로 다른 작성자가 서로 다른 목적으로 작성합니다.

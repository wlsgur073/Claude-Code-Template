...
4. **점진적 심화** — Day 1은 2분 설정. Day 30은 감사와 보안 강화 추가. Day 100은 스킬 간 메모리와 자동 드리프트 탐지가 활성화됩니다. 도구가 사용자와 함께 성장하며, 필요하지 않은 복잡성에 대한 비용은 내지 않습니다.

## Day 1 — 2분 빠른 시작

> **필수 조건:** Claude Code 설치 (`claude --version`으로 확인).
> **Windows**에서는 플러그인의 SessionStart 훅과 advanced 템플릿의 `UserPromptSubmit` 훅 모두 bash + PowerShell 두 엔트리를 함께 제공하므로, **PowerShell 5.1+ (Windows 10+ 기본 탑재) 또는 Git Bash/WSL** 중 어느 쪽이든 양쪽 레이어가 모두 동작합니다 — 추가 설정 불필요.
...
- **프로젝트 프로파일** — 자동 탐지된 기술 스택, 구조, 설정 상태 (`profile.json`, 사람이 읽을 수 있는 뷰는 `state-summary.md`)
...
v2.10.x를 사용하셨다면, 상태(state) 형식이 별도의 Markdown 파일에서 JSON을 source of truth로 사용하는 형태로 변경되었습니다. 업그레이드 후 처음 스킬을 실행할 때 `local/project-profile.md`와 `local/latest-*.md`가 `local/profile.json` + `local/recommendations.json`으로 자동 변환됩니다. 사람이 읽을 수 있는 단일 뷰 `local/state-summary.md`가 기존 MD 파일을 대체합니다. 원본은 `local/legacy-backup/<ISO-8601-UTC>/` 아래에 보존됩니다.

파싱에 실패한 파일이 있으면 스킬은 빈 상태로 계속 진행하며, Learning Rules(PENDING 카운트, DECLINED 이력)는 해당 실행 시점부터 다시 누적됩니다. **카운트를 수동으로 복원하려면** `legacy-backup`을 참고하세요 — 예를 들어 `legacy-backup`의 `latest-audit.md`에 PENDING 항목이 2개 있다면 `/audit`을 다시 실행하면 그 항목들이 새 PENDING 엔트리로 다시 표면화됩니다(카운터는 1부터 재시작하며, 자동 인계되지 않습니다).

**단방향 마이그레이션:** `local/profile.json`이 존재하면 MD 우선 상태로 자동 되돌리는 경로는 없습니다. 롤백은 `local/legacy-backup/<ISO-8601-UTC>/`에서 수동 복원 + v2.10.x 고정이 필요합니다.
...
│   │   └── plugin.json
│   ├── hooks/
│   │   ├── hooks.json       ← SessionStart 훅 (bash + powershell 듀얼 엔트리)
│   │   ├── session-start.sh ← bash 상태 점검 (Linux/macOS/Git Bash/WSL)
│   │   └── session-start.ps1 ← PowerShell 포트 (Windows 10+)
│   ├── references/

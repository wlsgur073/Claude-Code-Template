...
description: Per-sub-package score rollup procedure for /audit Phase 3.6 hook output. Defines coverage counters (4 fields), min/median/worst computation over scored_count, median even-count handling, worst tie semantics, n/a rendering when scored_count=0, and runtime invariants. No aggregate score (DEC-7).
version: 1.0.0
...
This document defines the rollup output for /audit Phase 3.6 hook (`Detection-Spec-2b.md` §5 L591 `emit_rollup_summary(min, median, worst, counts)`). The rollup is **computed on render** — no `min`, `median`, `worst` fields are persisted in `profile.json` (DEC-7 + R1 D4.4).

## §0. Scope

**Authority**: DEC-7 (no aggregate score; min/median/worst/counts + per-package score rows). Coverage counters ARE persisted (`subpackage_coverage` schema 1.2.0). Statistical aggregates (min/median/worst) are computed-on-render from `subpackages[].final_score`.
...
- **Even count**: **average** of two middle values (R1 D4.2 Accept frame (b)). Schema permits float (`final_score: number`), so `(60 + 71) / 2 = 65.5` is valid.

When `scored_count == 0`, `median = n/a`.

### §2.3 Worst (R1 P3 substitution)
...
Coverage counters still rendered. Two example scenarios for `scored_count == 0`:

- **Parse-error scenario** (with_claude_md=2, scored_count=0): `Rollup: min=n/a, median=n/a, worst=n/a (0 scored, 0 without CLAUDE.md, 2 unscored)` — both sub-packages had CLAUDE.md but failed to parse.
- **No-CLAUDE.md scenario** (with_claude_md=0, scored_count=0): `Rollup: min=n/a, median=n/a, worst=n/a (0 scored, 2 without CLAUDE.md, 0 unscored)` — sub-packages discovered via workspace declaration but none have CLAUDE.md yet.

The abstract rollup line template: `Rollup: min=n/a, median=n/a, worst=n/a (0 scored, M without CLAUDE.md, K unscored)` where `M = subpackage_coverage.without_claude_md` and `K = with_claude_md - scored_count`.

## §4. No Persistence (R1 D4.4)

`min`, `median`, `worst`, K (unscored = with_claude_md - scored_count) are **NOT** persisted in `profile.json`. They are computed on each terminal render from `subpackages[]` + `subpackage_coverage`. Schema 1.2.0 `subpackage_coverage` has only the 4 counter fields — adding statistical aggregates would require schema bump (out of scope for v2.13.0; DEC-7 binding).
